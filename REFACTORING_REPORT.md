...
### 1. **Syntax Error in `src/store/store.ts`**
**Location:** Lines 15-16
**Issue:** Line break in the `reducer` property assignment
```typescript
reducer: 
  rootReducer,
```
**Fix:** Should be `reducer: rootReducer,`

### 2. **Missing Environment Variables**
**Issue:** `.env` file is missing but referenced in code (`@env` imports)
**Impact:** App will fail at runtime when trying to access `PERENUAL_API_KEY`
**Fix:** 
...
**Location:** `src/components/auth/AuthForm.tsx` lines 21-25
**Issue:** Default form values are hardcoded test credentials
```typescript
useState(user?.email ?? 'test@gmail.com')
useState('test@gmail.com')
useState('password')
```
**Fix:** Remove default test values or make them conditional on development mode only
...
- [ ] Fix `store.ts` syntax error
- [ ] Create `.env` file with `PERENUAL_API_KEY`
- [ ] Remove test credentials from `AuthForm.tsx`
